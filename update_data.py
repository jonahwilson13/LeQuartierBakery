...
# The dashboard only ever shows a rolling window of this many most-recent
# weeks. Adding a new week beyond this drops the oldest one automatically.
MAX_WEEKS = 8
CATEGORY_MAP = {
...
def build_known_item_categories(existing_weeks):
    # Learns each item's usual Bread/Pastry/Dessert category from a
    # location's own history. This is what rescues rows where Toast leaves
    # the Sales Category blank on just one day (seen on Dundee, blank only
    # on Thursdays, for a scattered set of otherwise-normal items) — the
    # same item is correctly tagged on its other days, so that's used as
    # the fallback instead of just dropping the row.
    known = {}
    for week in existing_weeks:
        for cat in ("Breads", "Pastries", "Desserts"):
            for item in week.get("data", {}).get(cat, {}):
                known[item.lower()] = cat
    return known


def parse_toast_csv(path, known_item_categories=None):
    known_item_categories = known_item_categories or {}
    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not bucket and cat_raw == "":
                bucket = known_item_categories.get(item.lower())
            if "bagel" in item.lower():
                bucket = "Pastries"
            if not bucket:
                if cat_raw == "":
                    dropped_blank.append(f"{item} ({day}, qty {qty:g})")
                continue
            week_data[bucket].setdefault(item, {})
            week_data[bucket][item][day] = week_data[bucket][item].get(day, 0.0) + qty
            matched += 1
    return week_data, matched, dropped_blank
...
def main():
    ...
    known_item_categories = build_known_item_categories(weeks)
    week_data, matched, dropped_blank = parse_toast_csv(args.csv, known_item_categories)
    if matched == 0:
    ...
    if dropped_blank:
        print(f"Warning: {len(dropped_blank)} row(s) had a blank Sales Category with no known "
              f"item match, and were skipped rather than guessed at:", file=sys.stderr)
        for line in dropped_blank[:20]:
            print(f"  - {line}", file=sys.stderr)
        if len(dropped_blank) > 20:
            print(f"  ...and {len(dropped_blank) - 20} more", file=sys.stderr)
    ...
    if len(weeks) > MAX_WEEKS:
        dropped_weeks = [w["date"] for w in weeks[:-MAX_WEEKS]]
    ...
    if dropped_weeks:
        print(f"Dropped oldest week(s) to stay at the {MAX_WEEKS}-week rolling window: {', '.join(dropped_weeks)}")
    print("Next steps:")
