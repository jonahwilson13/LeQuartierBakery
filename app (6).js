...
// them out by day), so they get their own simple computation: total sold
// across the loaded weeks, ranked most to least popular.
function computeBreakfastLunch(weeksSorted) {
  ...
  return Object.entries(totals)
    .map(([item, total]) => ({ item, total: Math.round(total * 10) / 10 }))
...
function Dashboard({ user, onLogout }) {
  ...
  const blItems = useMemo(() => computeBreakfastLunch(weeks), [weeks]);
          ...
          category === "Breakfast & Lunch"
            ? blItems.length === 0
              ...
              : React.createElement("div", { style: { background: "#FBF6EC", border: "1px solid #DCCDA8", borderRadius: 14, padding: "20px 20px 8px", boxShadow: "0 1px 3px rgba(58,42,30,0.06)" } },
                  ...
                  React.createElement("div", { style: { fontSize: 11.5, color: "#9C8A6E", marginBottom: 14 } }, `Total units sold, all loaded weeks combined (${weeks.length} weeks) — no day-of-week breakdown available for this category yet.`),
                  React.createElement(PopularityBarChart, { items: blItems })
                )
            : React.createElement(React.Fragment, null,
