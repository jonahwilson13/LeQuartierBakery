...
// Order/Received/Waste production spreadsheets, hand-filled at each store,
// covering many more weeks of history than Toast (18-26 weeks vs 9). Used
// only for the two adjustments described in generateOrderComparisonNote —
// never as a wholesale replacement for the Toast sales numbers.
const WASTE_FILES = {
  Edgewood: "data/edgewood_waste.json",
  Meridian: "data/meridian_waste.json",
  Dundee: "data/dundee_waste.json",
  Loveland: "data/loveland_waste.json",
};
...
// Forecasting: low/medium/high for a chosen item — by day of week, the
// week's total, and — for desserts specifically, since that's the most
// waste-sensitive category — a 4-week (month) total too.
...
function WeekForecastPanel({ weekLabel, weekIdx, entry, days, isOpen, onToggle, isLast }) {
  return React.createElement("div", { style: { borderBottom: isLast ? "none" : "1px solid #F0DEDE" } },
    React.createElement("div", {
      ...
      React.createElement("span", { style: { fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 14, color: "#1A1A1A" } }, `Week of ${weekLabel} Forecast`),
      React.createElement("span", { style: { fontSize: 13, color: "#C41230", fontWeight: 700 } }, isOpen ? "\u2212" : "+")
    ),
    isOpen && React.createElement("div", { style: { padding: "2px 4px 16px" } },
      ...
      React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 5, marginBottom: 14 } },
        days.map((day) => {
          ...
          return React.createElement(ForecastRow, { key: day, label: day, low: df.low[weekIdx], med: df.med[weekIdx], high: df.high[weekIdx] });
        })
      ),
      ...
      React.createElement(ForecastRow, { label: `Week of ${weekLabel}`, low: entry.forecast_low[weekIdx], med: entry.forecast_med[weekIdx], high: entry.forecast_high[weekIdx], bold: true })
    )
  );
}
...
function ForecastingView({ location, derived, forecastCategory, setForecastCategory, forecastItem, setForecastItem }) {
  ...
  return React.createElement(React.Fragment, null,
    React.createElement("p", { style: { fontSize: 12.5, color: "#4D4D4D", maxWidth: 680, lineHeight: 1.55, margin: "0 0 18px" } },
      "Medium is a trend-line projection from the item's sales history; Low and High mark a realistic range around it based on how much that item's actual sales have varied week to week. Treat Medium as the planning number and the range as how much cushion to build in."
    ),
    ...
    React.createElement("div", { style: { background: "#FFFFFF", border: "1px solid #E8B9BC", borderRadius: 14, padding: "18px 20px", marginBottom: 18 } },
      ...
      React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 7 } },
        itemNames.map((name) => {
          ...
          return React.createElement("div", {
            key: name, className: "lq-item-chip", onClick: () => selectItem(active ? null : name),
            style: {
    ...
    !entry
      ...
      : React.createElement("div", { style: { background: "#FFFFFF", border: "1px solid #E8B9BC", borderRadius: 14, padding: "18px 20px 8px", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" } },
          ...
          React.createElement("div", { style: { fontSize: 11.5, color: "#7A7A7A", marginBottom: 6 } }, "Low / medium / high units to make — next 4 weeks, click a week to open it"),

          entry._wasteAdjustedDays && entry._wasteAdjustedDays.size > 0 && React.createElement("div", {
            style: { background: "#FFF6E5", border: "1px solid #E8CDA0", borderRadius: 8, padding: "8px 12px", fontSize: 11.5, color: "#5C4A1A", marginBottom: 10 }
          },
            React.createElement("strong", null, "Adjusted up: "),
            `${Array.from(entry._wasteAdjustedDays).join(", ")} — this item's production log shows it sold out before close on some of those days, so the forecast is nudged above raw sales history for those days specifically. See Data Notes for how.`
          ),

          entry.orderComparison && React.createElement("div", {
            style: { background: "#F0F7F5", border: "1px solid #B9D9CE", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#1A1A1A", marginBottom: 14 }
          },
            React.createElement("div", { style: { fontWeight: 700, marginBottom: 3 } }, "Recommended vs. actual ordering"),
            React.createElement("div", null,
              `Forecast (from sales history): ~${entry.orderComparison.forecastPerWeekEquiv}/week. `,
              `Recent actual ordering (from the production log): ~${entry.orderComparison.avgOrderPerWeek}/week`,
              entry.orderComparison.groupLabel !== forecastItem ? ` (tracked together with ${entry.orderComparison.groupLabel.split(" + ").filter((n) => n !== forecastItem).join(", ")} on one production line).` : "."
            ),
            React.createElement("div", { style: { color: "#4D4D4D", marginTop: 4, fontSize: 11 } }, "A large gap here means ordering isn't following the forecast, not that the forecast is wrong — see Data Notes.")
          ),

          derived.forecastLabels.map((label, idx) => React.createElement(WeekForecastPanel, {
            key: idx, weekLabel: label, weekIdx: idx, entry, days,
            isOpen: openWeeks.has(idx), onToggle: () => toggleWeek(idx),
            isLast: idx === derived.forecastLabels.length - 1 && forecastCategory !== "Desserts",
          })),

          forecastCategory === "Desserts" && React.createElement("div", { style: { paddingTop: 16 } },
            React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#666666", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" } }, "Month Total (all 4 weeks combined)"),
            React.createElement(ForecastRow, { label: "4-Week Total", low: monthLow, med: monthMed, high: monthHigh, bold: true }),
            React.createElement("div", { style: { fontSize: 10.5, color: "#9C9C9C", marginTop: 8, paddingBottom: 4 } }, "Desserts get a longer view here since they're the most waste-sensitive category — a bad single-week estimate is more costly than for bread or pastry.")
          )
        )
  );
}
...
function SnapshotBarChart({ entry, days }) {
  const data = days.map((day) => {
    const weeklyArr = entry.day_weekly[day] || [];
    return {
      day: day.slice(0, 3),
      fullDay: day,
      value: entry.day_ma[day][entry.day_ma[day].length - 1],
      actual: weeklyArr[weeklyArr.length - 1],
    };
  });
  return React.createElement(ResponsiveContainer, { width: "100%", height: 200 },
    React.createElement(BarChart, { data, margin: { top: 22, right: 12, left: -10, bottom: 0 } },
      React.createElement(CartesianGrid, { stroke: "#F4DCDD", strokeDasharray: "0", vertical: false }),
      ...
      React.createElement(Bar, {
        ...
        // Two numbers above each bar, kept to a single readable line:
        // the 3-week average (bold, dark — what the bar height shows),
        // then the most recent single week's actual count (red) right next to it.
        label: (props) => {
          const { x, y, width, value, index } = props;
          const cx = x + width / 2;
          const actual = data[index].actual;
          return React.createElement("text", { x: cx, y: y - 8, textAnchor: "middle", fontSize: 12, fontFamily: "'Inter', sans-serif" },
            React.createElement("tspan", { fill: "#1A1A1A", fontWeight: 700 }, value?.toFixed(1)),
            actual !== undefined && React.createElement("tspan", { fill: "#C41230", fontWeight: 600, dx: 4 }, `(${actual})`)
          );
        },
      })
    )
...
function ItemDayRow({ name, entry, visibleDays }) {
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", { style: { fontSize: 11.5, color: "#666666", marginBottom: 2 } }, "Current 3-week moving average, side by side by day of week"),
    React.createElement("div", { style: { fontSize: 10.5, color: "#999999", marginBottom: 10 } },
      React.createElement("span", { style: { color: "#1A1A1A", fontWeight: 700 } }, "Bold"), " = 3-week average  \u00b7  ",
      React.createElement("span", { style: { color: "#C41230", fontWeight: 600 } }, "(Red)"), " = most recent week's actual"
    ),
    days.length === 0
...
function ItemDetailCard({ name, entry, color, labels, forecastLabels }) {
  ...
  return React.createElement("div", {
    ...
    React.createElement("div", { style: { fontSize: 11.5, color: "#7A7A7A", marginTop: 12 } },
      "For a production estimate (low / medium / high, by day, next 4 weeks) see the ",
      React.createElement("strong", { style: { color: "#C41230" } }, "Forecasting"),
      " tab."
    )
  );
}
...
async function loadWasteForLocation(loc) {
  try {
    const res = await fetch(WASTE_FILES[loc], { cache: "no-store" });
    if (!res.ok) return [];
...
// Finds an item's bucket entry regardless of which category (Breads /
// Pastries / Desserts) it lives in — the waste spreadsheet mapping doesn't
// track category, just item names.
function findItemEntry(buckets, itemName) {
  for (const cat of DAY_CATEGORIES) {
    if (buckets[cat] && buckets[cat][itemName]) return buckets[cat][itemName];
  }
  return null;
}

// Recomputes an item's overall (all-days-combined) forecast by summing its
// already-corrected per-day forecasts, so the headline number in the
// Forecasting tab always matches what the day-by-day panel shows below it —
// rather than the two drifting out of sync after a day-level correction.
function resumTotalForecast(entry) {
  const days = Object.keys(entry.day_forecast);
  ["forecast_med", "forecast_low", "forecast_high"].forEach((key) => {
    const dayKey = key.replace("forecast_", "");
    entry[key] = entry[key].map((_, weekIdx) =>
      Math.round(days.reduce((sum, d) => sum + entry.day_forecast[d][dayKey][weekIdx], 0) * 10) / 10
    );
  });
}

// Applies two adjustments from the Order/Received/Waste spreadsheets, and
// only these two — see the paragraph in README.md / the Data Notes section
// for the reasoning behind not doing more than this:
//
// 1. Sell-out bump: if an item historically sold out before close on a
//    given day, real demand that day was higher than the recorded sale
//    (Toast can only record what it had in stock to sell). The forecast
//    for that item/day gets nudged up, scaled by how often it happened.
//
// 2. Order-vs-forecast comparison: rather than trying to blend the
//    spreadsheet's Order/Waste numbers into the forecast itself (which
//    turned out not to hold up under testing — see Data Notes), the
//    comparison is surfaced directly: "the forecast says X, recent actual
//    ordering has been Y" — left for a person to act on rather than
//    silently folded into the number.
function applyWasteInsights(buckets, wasteGroups) {
  wasteGroups.forEach((group) => {
    const weight = group.weight || {};
    const weightOf = (item) => (weight[item] !== undefined ? weight[item] : 1);

    // For seasonal pairs (e.g. Apple Pecan / Cranberry Walnut sharing one
    // production line), only touch whichever item actually has recent
    // sales — the dormant half of the pair is correctly near-zero already.
    let targetItems = group.canonicalItems;
    if (group.seasonalPick) {
      const recentTotal = (itemName) => {
        const e = findItemEntry(buckets, itemName);
        if (!e) return 0;
        return e.total_weekly.slice(-4).reduce((a, b) => a + b, 0);
      };
      targetItems = group.canonicalItems.filter((i) => recentTotal(i) > 0);
      if (targetItems.length === 0) targetItems = group.canonicalItems; // neither active recently; leave both alone below (entries just won't be found or will no-op)
    }

    // --- 1. Sell-out bump, applied per item/day ---
    Object.entries(group.days).forEach(([day, stats]) => {
      if (stats.weeksTracked < 3) return; // too little history to trust
      const sellOutRate = stats.sellOutWeeks / stats.weeksTracked;
      if (sellOutRate <= 0) return;
      const bump = Math.min(0.10 + 0.30 * sellOutRate, 0.30); // 10%-30% bump, scaling with how often it sold out
      targetItems.forEach((itemName) => {
        const entry = findItemEntry(buckets, itemName);
        if (!entry || !entry.day_forecast[day]) return;
        const df = entry.day_forecast[day];
        ["med", "low", "high"].forEach((k) => {
          df[k] = df[k].map((v) => Math.round(v * (1 + bump) * 10) / 10);
        });
        entry._wasteAdjustedDays = entry._wasteAdjustedDays || new Set();
        entry._wasteAdjustedDays.add(day);
      });
    });

    // Re-sum the weekly total for any item touched above, so it stays
    // consistent with the corrected day-by-day figures.
    targetItems.forEach((itemName) => {
      const entry = findItemEntry(buckets, itemName);
      if (entry && entry._wasteAdjustedDays && entry._wasteAdjustedDays.size > 0) {
        resumTotalForecast(entry);
      }
    });

    // --- 2. Order-vs-forecast comparison note ---
    const avgOrderPerWeek = Object.values(group.days).reduce((sum, d) => sum + d.avgOrder, 0);
    const daysTracked = Object.keys(group.days);
    if (avgOrderPerWeek <= 0 || daysTracked.length === 0) return;

    let forecastPerWeekEquiv = 0;
    targetItems.forEach((itemName) => {
      const entry = findItemEntry(buckets, itemName);
      if (!entry) return;
      const w = weightOf(itemName);
      daysTracked.forEach((day) => {
        if (entry.day_forecast[day]) forecastPerWeekEquiv += w * entry.day_forecast[day].med[0];
      });
    });
    forecastPerWeekEquiv = Math.round(forecastPerWeekEquiv * 10) / 10;

    const note = {
      groupLabel: targetItems.join(" + "),
      avgOrderPerWeek: Math.round(avgOrderPerWeek * 10) / 10,
      forecastPerWeekEquiv,
      daysTracked: daysTracked.length,
    };
    targetItems.forEach((itemName) => {
      const entry = findItemEntry(buckets, itemName);
      if (entry) entry.orderComparison = note;
    });
  });
}
...
// Plain-language explanation of where every number on the site comes from
// and how far each data source can be trusted — lives on the Home page so
// it applies once, across all four locations, rather than repeating per tab.
function DataNoteBlock({ title, children }) {
  return React.createElement("div", { style: { marginBottom: 16 } },
    React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 13.5, color: "#C41230", marginBottom: 4 } }, title),
    React.createElement("div", { style: { fontSize: 12.5, lineHeight: 1.6, color: "#333333" } }, children)
  );
}

function DataNotesSection() {
  return React.createElement("div", { style: { borderTop: "1px solid #E8B9BC", paddingTop: 24, marginTop: 24 } },
    React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 16, color: "#1A1A1A", marginBottom: 4 } }, "Data Notes"),
    React.createElement("div", { style: { fontSize: 12, color: "#666666", marginBottom: 16 } }, "Where every number on this site comes from, and how far it can be trusted."),

    React.createElement("div", { style: { background: "#FFFFFF", border: "1px solid #E8B9BC", borderRadius: 10, padding: "18px 20px" } },

      React.createElement(DataNoteBlock, { title: "Item Sales & Forecasts (Breads, Pastries, Desserts, Breakfast & Lunch, After 2PM, and the \u201CMedium\u201D forecast line)" },
        "Straight from Toast's own Product Mix (PMIX) report \u2014 exactly what rang up at the register, by item, by day. The moving averages and 4-week forecasts are pure math on top of this real sales history: a trend line fit to actual sales, with a range above and below based on how much that item has historically varied week to week. No estimates, no manual entry."
      ),

      React.createElement(DataNoteBlock, { title: "Hourly Demand tab" },
        "A different Toast report (Sales Summary, by time of day) rather than PMIX \u2014 total sales by hour, not broken out by item. Each day's line is an average across the weeks of data loaded, so it reflects a typical Monday, typical Saturday, etc., not one single date."
      ),

      React.createElement(DataNoteBlock, { title: "\u201CAdjusted up\u201D notes on the Forecasting tab" },
        "Each location keeps a hand-filled Order/Received/Waste production log, going back further than Toast's own history. If a log shows an item sold out before close fairly often on a given day, that means real demand was higher than what Toast recorded \u2014 Toast can only count what was in stock to sell. On those specific item/days, the forecast is nudged upward to account for that. Nothing else changes."
      ),

      React.createElement(DataNoteBlock, { title: "\u201CRecommended vs. actual ordering\u201D notes" },
        "The production logs are deliberately NOT used to pull forecasts down, even though that was the first instinct. Testing it against Edgewood's bagels (54/week ordered vs. ~24/week actually sold) showed why: the sales-based forecast is already close to the real number \u2014 the gap isn't a flaw in the forecast, it's that ordering has been running on habit, disconnected from what the dashboard says. So instead of quietly blending two different numbers together, both are shown side by side \u2014 what the forecast recommends, and what's actually been ordered recently \u2014 so the gap is visible and actionable rather than papered over."
      ),

      React.createElement("div", { style: { fontSize: 12, fontWeight: 600, color: "#1A1A1A", marginTop: 4, paddingTop: 12, borderTop: "1px solid #F0DEDE" } },
        "Bottom line: sales numbers and forecasts are 100% Toast data. The production logs only ever nudge a forecast up, never down, and are always shown as a transparent side-by-side comparison rather than folded invisibly into the math."
      )
    )
  );
}
    ...
    (async () => {
      ...
      for (const loc of LOCATION_NAMES) {
        ...
        weeks.forEach((w) => {
          ...
          if (Object.keys(w.data["Breakfast & Lunch"] || {}).length > 0) blWeeks += 1;
          if (Object.keys(w.data["After 2PM"] || {}).length > 0) a2Weeks += 1;
        });
        results[loc] = { weeksLoaded: weeks.length, total: Math.round(total) };

        const hourly = await loadHourlyForLocation(loc);

        status[loc] = {
          items: weeks.length,
          breakfastLunch: blWeeks,
          after2pm: a2Weeks,
          hourly: hourly.length,
        };
      }
      if (!cancelled) { setSummaries(results); setCompleteness(status); }
    })();
    return () => { cancelled = true; };
  }, []);

  const StatusCell = ({ count, label }) => {
    const has = count > 0;
    return React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
      React.createElement("span", {
        style: {
          ...
          background: has ? "#1A8B4A" : "#E8B9BC",
        }
      }),
      React.createElement("span", { style: { fontSize: 12, color: has ? "#1A1A1A" : "#9C9C9C" } }, has ? `${label}: ${count}` : `${label}: none`)
    );
  };

  const businessTotal = summaries
    ? LOCATION_NAMES.reduce((sum, loc) => sum + (summaries[loc] ? summaries[loc].total : 0), 0)
    : 0;
  const maxWeeksLoaded = summaries
    ? Math.max(...LOCATION_NAMES.map((loc) => (summaries[loc] ? summaries[loc].weeksLoaded : 0)))
    : 0;

  return React.createElement("div", null,
    React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontSize: 12.5, letterSpacing: "0.14em", textTransform: "uppercase", color: "#C41230", fontWeight: 600, marginBottom: 8 } }, "Operations Overview"),
    React.createElement("h1", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 40, lineHeight: 1.1, margin: "0 0 8px", color: "#1A1A1A" } }, "Le Quartier"),
    React.createElement("p", { style: { fontSize: 14, color: "#333333", margin: "0 0 26px" } }, "Select a location for its full breakdown, or compare all four here first."),
    !summaries
      ...
      : React.createElement(React.Fragment, null,
          ...
          React.createElement("div", {
            style: {
              background: "#1A1A1A", borderLeft: "6px solid #C41230", borderRadius: 10,
              padding: "18px 24px", marginBottom: 24,
              display: "flex", flexWrap: "wrap", gap: "10px 40px", alignItems: "baseline",
            }
          },
            React.createElement("div", null,
              React.createElement("div", { style: { fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: "#D9A0A6", marginBottom: 4 } }, "All Locations Combined"),
              React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 30, color: "#FFFFFF" } }, businessTotal.toLocaleString(), React.createElement("span", { style: { fontSize: 14, fontWeight: 500, color: "#D9A0A6", marginLeft: 8 } }, "units sold"))
            ),
            React.createElement("div", null,
              React.createElement("div", { style: { fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: "#D9A0A6", marginBottom: 4 } }, "Locations Reporting"),
              React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 30, color: "#FFFFFF" } }, LOCATION_NAMES.filter((l) => summaries[l] && summaries[l].weeksLoaded > 0).length, React.createElement("span", { style: { fontSize: 14, fontWeight: 500, color: "#D9A0A6", marginLeft: 8 } }, "of 4"))
            ),
            React.createElement("div", null,
              React.createElement("div", { style: { fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: "#D9A0A6", marginBottom: 4 } }, "Latest Data Window"),
              React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 30, color: "#FFFFFF" } }, maxWeeksLoaded, React.createElement("span", { style: { fontSize: 14, fontWeight: 500, color: "#D9A0A6", marginLeft: 8 } }, "weeks"))
            )
          ),
          ...
          React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16, marginBottom: 32 } },
            LOCATION_NAMES.map((loc) => {
              ...
              return React.createElement("div", {
                ...
                style: {
                  background: "#FFFFFF", border: "1px solid #E8B9BC", borderLeft: "5px solid #C41230",
                  borderRadius: 10, padding: "20px 22px", cursor: "pointer",
                }
              },
                React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 19, color: "#1A1A1A", marginBottom: 12 } }, loc),
                noData
                  ? React.createElement("div", { style: { fontSize: 12.5, color: "#7A7A7A" } }, "No data loaded yet")
                  : React.createElement(React.Fragment, null,
                      React.createElement("div", { style: { fontSize: 28, fontWeight: 700, color: "#C41230", fontFamily: "'Fraunces', serif", lineHeight: 1.1 } }, s.total.toLocaleString()),
                      React.createElement("div", { style: { fontSize: 12, color: "#666666", marginBottom: 10 } }, "units sold (Bread, Pastry, Dessert)"),
                      React.createElement("div", { style: { fontSize: 12, color: "#7A7A7A", borderTop: "1px solid #F0DEDE", paddingTop: 8 } }, `${s.weeksLoaded} week${s.weeksLoaded === 1 ? "" : "s"} of data loaded`)
                    )
              );
            })
          ),

          React.createElement("div", { style: { borderTop: "1px solid #E8B9BC", paddingTop: 24 } },
            React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 16, color: "#1A1A1A", marginBottom: 4 } }, "Data Completeness"),
            React.createElement("div", { style: { fontSize: 12, color: "#666666", marginBottom: 14 } }, "What's actually loaded for each location right now, across every part of the site."),
            React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 } },
              LOCATION_NAMES.map((loc) => {
                const c = completeness && completeness[loc];
                return React.createElement("div", {
                  key: loc,
                  style: { background: "#FFFFFF", border: "1px solid #E8B9BC", borderRadius: 10, padding: "14px 16px" }
                },
                  React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 14, color: "#1A1A1A", marginBottom: 8 } }, loc),
                  !c
                    ? React.createElement("div", { style: { fontSize: 12, color: "#9C9C9C" } }, "Checking\u2026")
                    : React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 5 } },
                        React.createElement(StatusCell, { count: c.items, label: "Bread/Pastry/Dessert (weeks)" }),
                        React.createElement(StatusCell, { count: c.breakfastLunch, label: "Breakfast & Lunch (weeks)" }),
                        React.createElement(StatusCell, { count: c.after2pm, label: "After 2PM (weeks)" }),
                        React.createElement(StatusCell, { count: c.hourly, label: "Hourly demand (days)" })
                      )
                );
              })
            )
          ...
          React.createElement(DataNotesSection, null)
        )
  );
}

// Big, unmissable banner at the top of each location's page showing exactly
// which weeks of data are currently loaded — so a stale or gapped dataset
// is obvious without having to dig into any tab first.
function WeeksLoadedBanner({ weeks }) {
  ...
  const rangeLabel = firstYear === lastYear
    ? `${formatDateLabel(first)} \u2013 ${formatDateLabel(last)}, ${lastYear}`
    : `${formatDateLabel(first)}, ${firstYear} \u2013 ${formatDateLabel(last)}, ${lastYear}`;

  return React.createElement("div", {
    style: {
      background: "#FFF6E5", border: "1.5px solid #E8CDA0", borderRadius: 12,
      padding: "14px 20px", marginBottom: 24,
    }
  },
    React.createElement("div", { style: { fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 22, color: "#1A1A1A", lineHeight: 1.2 } },
      `Data loaded: ${rangeLabel}`
    ),
    React.createElement("div", { style: { fontSize: 12, color: "#7A6A4A", marginTop: 4 } },
      `${sorted.length} week${sorted.length === 1 ? "" : "s"} \u00b7 ${sorted.map((w) => formatDateLabel(w.date)).join(", ")}`
    )
  );
}
...
function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const g = await loadWasteForLocation(location);
      if (!cancelled) setWasteGroups(g);
    })();
    return () => { cancelled = true; };
  }, [location]);
  ...
  const derived = useMemo(() => {
    const d = computeAllDerived(weeks);
    if (wasteGroups.length > 0) applyWasteInsights(d.buckets, wasteGroups);
    return d;
  }, [weeks, wasteGroups]);
    ...
    React.createElement("div", { style: { maxWidth: 1080, margin: "0 auto" } },
      React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 } },
        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 14 } },
          React.createElement("span", {
            style: {
              fontFamily: "'Fraunces', serif", fontSize: 13, letterSpacing: "0.16em", textTransform: "uppercase",
      ...
      : React.createElement(React.Fragment, null,
          React.createElement(WeeksLoadedBanner, { weeks }),
